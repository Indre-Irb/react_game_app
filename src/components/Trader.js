...
import {addEquipment} from "../features/equipment";
import {useDispatch, useSelector} from "react-redux";
import {updatePlayerGold} from "../features/status";
...
const Trader = () => {
    ...
    const equipmentList = useSelector(state => state.equipment.value)
    const products = useSelector(state => state.newArray.value)
    ...
    function showWeapon() {
        trader.weapons.map(weapon => {
            const newWeapon = []
            weapon.effects.map(x => {
                newWeapon.push(effects[x])
            })
            weapon.effects = [...newWeapon]
            setItem(trader.weapons)
        })
    }


    function showPotions() {
        setItem(trader.potions)
    }

    function boughtItem(stuff) {
        if (hero.inventorySlots > equipmentList.length && hero.gold >= stuff.price) {
            dispatch(updatePlayerGold(stuff.price))
            dispatch(addEquipment(stuff))
        }
    }
    ...
    return (
        <div className="storeInfo">
            <div className="d-flex">
                <div className="flex1 storeCategory border-radius-tl" onClick={() => showWeapon()}>Weapon</div>
                <div className="flex1 storeCategory" onClick={() => showPotions()}>Potion</div>
            </div>
            <div className="d-flex f-wrap j-center scroll">
                {getItem.map((item, index) =>
                    <div key={index} className="shopItem" onClick={() => boughtItem(item)}>
                        ...
                        <div>
                            {(item.effects) ?
                                <div>
                                    ...
                                    <h5>Energy per hit: {item.energyPerHit}</h5>
                                    <h5>Max damage: {item.maxDamage}</h5>
                                </div>
                                :
                                <h5>Effect: {item.title}</h5>}
                            <h3>Price: {item.price} gold</h3>
                        </div>
                    </div>)}
            </div>
        </div>
